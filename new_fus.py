...
# Combine LSTM and LGBM features
attention_input_size = lstm_feature_representation_transposed.shape[1] + x_train.shape[1]
attention_output_size = 189  # Modify this line to match the number of input sequences
attention_model = AttentionModel(attention_input_size, attention_output_size)
criterion_attention = nn.BCELoss()
optimizer_attention = optim.Adam(attention_model.parameters(), lr=0.01)
...
# for epoch in range(num_epochs_attention):
#     optimizer_attention.zero_grad()
#     outputs_attention = attention_model(torch.FloatTensor(combined_features))
#
#     # Apply sigmoid activation
#     outputs_attention_sigmoid = torch.sigmoid(outputs_attention)
#
#     # Modify the target size to match the output size of the attention model
#     # y_train_torch_expanded = y_train_torch.view(-1, 1).expand_as(outputs_attention_sigmoid)
#     # y_train_torch_expanded = y_train_torch.view(-1, 1).repeat(1, outputs_attention_sigmoid.size(1))
#     y_train_torch_expanded = y_train_torch.view(-1, 1).expand(-1, outputs_attention_sigmoid.size(1))
#
#     loss_attention = criterion_attention(outputs_attention_sigmoid.squeeze(), y_train_torch_expanded)
#     loss_attention.backward()
#     optimizer_attention.step()
#
# # Evaluate the Attention Model on Test Set
# with torch.no_grad():
#     attention_model.eval()
#     outputs_attention_test = attention_model(torch.FloatTensor(combined_features))
...
#
#     # Modify the target size to match the output size of the attention model
#     # y_test_torch_expanded = y_test_torch.view(-1, 1).expand_as(outputs_attention_test_sigmoid)
#     y_test_torch_expanded = y_test_torch.view(-1, 1).repeat(1, outputs_attention_test_sigmoid.size(1))
#
#     loss_attention_test = criterion_attention(outputs_attention_test_sigmoid, y_test_torch_expanded)
#
#     accuracy_attention_test = accuracy_score(y_test[:len(predictions_attention_test)], predictions_attention_test)
#     precision_attention_test = precision_score(y_test[:len(predictions_attention_test)], predictions_attention_test)
#     recall_attention_test = recall_score(y_test[:len(predictions_attention_test)], predictions_attention_test)
...
#
#     # Confusion matrix
#     confusion_matrix_attention = confusion_matrix(y_test[:len(predictions_attention_test)], predictions_attention_test)
#
#     print(f"Attention Model Accuracy: {accuracy_attention_test}")
#     print(f"Attention Model Precision: {precision_attention_test}")
#     print(f"Attention Model Recall: {recall_attention_test}")
...
for epoch in range(num_epochs_attention):
    ...
    # Modify the target size to match the output size of the attention model
    y_train_torch_expanded = y_train_torch.view(-1, 1)[:outputs_attention.size(0), :].squeeze()
...
with torch.no_grad():
    ...
    # Calculate the binary cross-entropy loss
    loss_attention_test = criterion_attention(outputs_attention_test_sigmoid, y_test_torch)
