...
with torch.no_grad():
    attention_model.eval()
    X_test_combined_torch = torch.FloatTensor(np.concatenate((lstm_model(X_test_torch).squeeze().numpy().reshape(-1, 1), x_test), axis=1))
...
# Define custom weights for class 0 and class 1
weight_class_0 = 2  # You can adjust this weight as needed
weight_class_1 = 1.5  # Default weight for class 1

# Test the Attention model on the new data with a custom threshold
custom_threshold = 0.05  # You can adjust this threshold as needed
...
# Evaluate the performance on the new data with custom weights
accuracy_new = accuracy_score(new_data_labels, attention_predictions_new)
precision_new = precision_score(new_data_labels, attention_predictions_new, pos_label=1, average='binary', sample_weight=None, zero_division='warn')
recall_new = recall_score(new_data_labels, attention_predictions_new, pos_label=1, average='binary', sample_weight=None, zero_division='warn')

# Apply custom weights for class 0
precision_weighted = precision_score(new_data_labels, attention_predictions_new, pos_label=1, average='binary', sample_weight=[weight_class_1 if label == 1 else weight_class_0 for label in new_data_labels], zero_division='warn')
recall_weighted = recall_score(new_data_labels, attention_predictions_new, pos_label=1, average='binary', sample_weight=[weight_class_1 if label == 1 else weight_class_0 for label in new_data_labels], zero_division='warn')

print(f"Accuracy on new data: {accuracy_new}")
print(f"Precision on new data: {precision_new}")
print(f"Recall on new data: {recall_new}")
print(f"Weighted Precision on new data: {precision_weighted}")
print(f"Weighted Recall on new data: {recall_weighted}")
