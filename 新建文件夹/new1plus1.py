...
new_data = pd.read_csv('D:\pythonProject\code\APIN_Ubuntu\APIN-master-org\data\AMP.eval_output28.csv')
# Convert sequences to numeric sequences for new_data
new_data['encoded_sequence'] = new_data['Sequences'].apply(lambda x: [char_to_int[char] for char in x])
...
new_data = pd.read_csv('D:\Desktop\AMP-EF-main\DBAASP - test_output28.csv')
# Convert sequences to numeric sequences for new_data
new_data['encoded_sequence'] = new_data['Sequences'].apply(lambda x: [char_to_int[char] for char in x])
...
new_data = pd.read_csv('D:\Desktop\AMP-EF-main\YADAMP - test_output28.csv')
# Convert sequences to numeric sequences for new_data
new_data['encoded_sequence'] = new_data['Sequences'].apply(lambda x: [char_to_int[char] for char in x])
...
print(f"Precision on new data: {precision_new}")
print(f"Recall on new data: {recall_new}")
split_index = int(0.7 * len(X_combined_torch_new))

threshold_first_90_percent = 0.2
threshold_last_10_percent = 0.7
